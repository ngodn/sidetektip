...
class Msinglelink extends CI_Model {
    ...
    public function singleLink($termFreqTable,$iParam) {
        ...
        $flag = array();
        for ($i = 0; $i < ($maxDoc); $i++){
            $nilaiTotal[$i] = 1; //cosine similarity tiap cluster
            $anggotaCluster[$i] = 1; //banyak anggota cluster
        }
        $loop = 0;
        do{
        //  while ($nilaiI > $iParam){  
            ...
            for ($i = 0; $i < ($maxDoc-1); $i++){
                for ($j = $i+1; $j < $maxDoc; $j++){ 
                    if ($arraySim[$i][$j] > $maxSim){
                        $maxSim = $arraySim[$i][$j]; //nyari dua cluster dengan similarity terbesar (paling mirip)
                        $d1 = $i; //intinya nyimpan si clusternya
                        $d2 = $j; // sama
                    }
                }
            }
            ...
            if ($d1 != -1){
                    ...
                    if (($kelompok[$i] != $kelompok[$d1]) or ($kelompok[$i] != $kelompok[$d2])){
                        for ($j = 0; $j < $maxDoc; $j++){
                            if (($kelompok[$j] == $kelompok[$d1]) or ($kelompok[$j] == $kelompok[$d2])){
                                if ($arraySim[$i][$d1] > $arraySim[$i][$d2]){ //mana cosine similarity yang lebih besar
                                    $arraySim[$i][$j] = $arraySim[$i][$d1]; 
                                    $arraySim[$j][$i] = $arraySim[$i][$j];
           
                                }
                                else{                                           //bagian looping ini buat membentuk cosine similarity baru setelah ada cluster yang digabung
                ...
                $kel2 = $kelompok[$d2];
                            ...
                            if (($kelompok[$i] == $kel1) or ($kelompok[$i] == $kel2)){
                                for ($j = 0; $j < $maxDoc; $j++){
                                    if (($kelompok[$j] == $kel1) or ($kelompok[$j] == $kel2)){                                        
                                        $kelompok[$j] = $kel2; //menggabungkan cluster yang cosine similaritynya tadi paling dekat
					$arraySim[$i][$j] = 0;
					$arraySim[$j][$i] = 0;
                                    }
                                }
                 ...
                 if ($anggotaCluster[$kel1] > $anggotaCluster[$kel2]){
                    ...
                    $anggotaCluster[$kel1] += $anggotaCluster[$kel2]; //menghitung nilai cosine similarity terbaru dari cluster yang baru terbentuk
            ...
            for ($z = 0; $z < $maxDoc; $z++){
                if ($flag[$kelompok[$z]] == false){
                    ...
                    $total += $nilaiTotal[$kelompok[$z]];  //memperbarui nilai I (yang jadi batas looping)
                    $count++;
            ...
            for ($z = 0; $z < $maxDoc; $z++){
                if ($flag2[$kelompok[$z]] == false){
                    ...
                    $clusters[$indeks[$kelompok[$z]]][] = $nrp[$z];  //merekam perubahan cluster dari awal
                }
            }
